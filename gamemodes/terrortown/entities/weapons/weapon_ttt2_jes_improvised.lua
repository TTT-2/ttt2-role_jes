...
if SERVER then
   ...
   CreateConVar("ttt2_jes_improvised", "1", FCVAR_NOTIFY, FCVAR_ARCHIVE)

   hook.Add("Initialize", "AddJESCrowToDefaultLoadout", function()
 		local wep = weapons.GetStored("weapon_ttt2_jes_improvised")

 		if wep then
 			wep.InLoadoutFor = wep.InLoadoutFor or {}

 			if not table.HasValue(wep.InLoadoutFor, ROLE_JESTER) then
 				table.insert(wep.InLoadoutFor, ROLE_JESTER)
 			end
 		end
 	end)
end

hook.Add("TTTUlxDynamicRCVars", "TTTUlxDynamicJesCrowCVars", function(tbl)
...
function SWEP:SecondaryAttack()
   ...
   if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self:GetOwner():EyePos() - tr.HitPos):Length() < 100 then
      ...
      if SERVER and (not ply:IsFrozen()) and GetConVar("ttt2_jes_improvised"):GetBool() then
...
function SWEP:GetClass()
	return "weapon_ttt2_jes_improvised"
end
